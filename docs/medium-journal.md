...
#### #413 — [BUG] Durcir le RSI en garde-fou d'éligibilité en mode dégradé

- **Branche** : `feat/issue-406-mode-degrade-rsi`
- **Mergée** : 06:57 UTC
- **Issue liée** : #406
- **Résumé** : Incident direct du cycle TRUMP du 22/08 à 20h14 (RSI 4h = 70,4 — suracheté). En mode dégradé (rate-limit TradingView 1D), le RSI était un simple bonus de score, pas un garde-fou. TRUMP a passé le filtre avec 4/10 et perdu 13,20 USDC en 4h, faisant repasser le PnL net cumulé de +11,11 USDC à -1,91 USDC. Correctif : en mode dégradé, le RSI devient une condition d'éligibilité — un coin suracheté (RSI hors zone [30-65]) est exclu même s'il dépasse le seuil de score abaissé. RSI inconnu = inéligible. Hors mode dégradé, le comportement RSI-bonus de la PR #380 reste inchangé. 4 nouveaux tests couvrent les cas clés (156 tests passent au total).

#### #412 — [M1] Compter les abandons du watcher maker

- **Branche** : `feat/issue-408-compteur-abandons`
- **Mergée** : 06:58 UTC
- **Issue liée** : #408
- **Résumé** : Le watcher maker pouvait abandonner un ordre (prix dévié > `price_deviation_max_pct`) sans laisser de trace persistante. Un message Telegram était envoyé, mais aucun compteur n'était incrémenté. Ajout de `total_abandoned` dans `state/maker_watcher_state.json`, sur le même modèle que `total_fills` et `total_fallbacks`. Rétrocompatibilité : les fichiers d'état existants sans ce champ utilisent un défaut à 0. Propagation via le tuple de retour des trois handlers de résolution. 4 nouveaux tests.

#### #415 — [BUG] Frais d'entrée manquants sur positions ouvertes et trades clôturés

- **Branche** : `feat/issue-409-frais-entree-manquants`
- **Mergée** : 06:59 UTC
- **Issue liée** : #409
- **Résumé** : `scripts/backfill_fees.py` sautait les positions encore ouvertes (7 trades signalés `trades_skipped_not_closed`). XBT et LINK, ouvertes avant le déploiement de #382, avaient un `exit_fee_usdc` capté mais un `entry_fee_usdc` à `None` — PnL net surestimé de ~0,12 et 0,09 USDC. Ajout de trois flux distincts dans `backfill()` : positions ouvertes (entrée seule, jamais de champs de clôture), trades clôturés incomplets (entrée + recalcul PnL net), flux nominal existant. Nouveau compteur `open_entries_backfilled`. 4 nouveaux tests.

#### #414 — [M1] Filtrer la liquidité sur le spread et sa persistance, pas un instantané

- **Branche** : `feat/issue-407-filtre-liquidite`
- **Mergée** : 07:05 UTC
- **Issue liée** : #407
- **Résumé** : Même incident TRUMP — le filtre volume instantané avait laissé passer un pic x27 en une nuit (21 617 → 586 836 USDC), avec un spread de 0,126 % contre 0,010 % pour XBT/SOL. Le stop-loss a glissé de 2,3 % à l'exécution. Deux couches ajoutées à la Phase 1 : (1) filtre spread `max_spread_pct = 0,08 %` (évalué depuis les champs `a`/`b` du ticker Kraken, sans appel réseau supplémentaire) ; (2) filtre de persistance — `kraken ohlc` vérifie que le volume tient sur ≥ 4 des 6 dernières périodes 4h, appelé uniquement sur les candidats passant déjà les filtres précédents. Deux nouveaux paramètres dans `config.json`. Refactoring `fake_kraken.py` en dispatch dict (REC-AUTO #416–#420, intégrées dans cette PR). 4 nouveaux tests intégration.

#### #422 — [BUG] phase6_next_cycle.py duplique fmt_local et next_4h_slot — heure décalée de 2h

- **Branche** : `feat/issue-421-phase6-fuseau`
- **Mergée** : 07:34 UTC
- **Issues liées** : #421, #395 (régression)
- **Résumé** : `phase6_next_cycle.py` réimplémentait son propre calcul du slot 4h et son propre `.astimezone()` sans argument. Sur la VPS (fuseau machine = `Etc/UTC`), le slot 08:05 UTC s'affichait `08:05 (heure locale)` au lieu de `10:05`. Le correctif #395 n'avait pas atteint ce script, qui n'importait rien de `core.timing`. Refactoring : le script appelle désormais `next_4h_slot()` et `fmt_local()` depuis `core.timing`, passant de 26 à 15 lignes. Test de régression ajouté pour vérifier que le slot s'affiche à l'heure Paris configurée, indépendamment du fuseau de la machine.

#### #423 — [TECH] Test de cohérence des contrats prompt ↔ script

- **Branche** : `feat/issue-403-test-coherence-prompt-script`
- **Mergée** : 08:46 UTC
- **Issue liée** : #403
- **Résumé** : Trois incidents en août 2026 (PRs #391, #397, bug #385) montraient qu'un outil de review automatique pouvait déplacer un chemin de fichier d'échange inter-phases sans savoir qu'il faisait partie d'un contrat implicite — causant des `FileNotFoundError` en prod. Nouveau fichier `tests/test_prompt_script_contract.py` (435 lignes) : analyse AST des 11 paires script/prompt, extrait statiquement les chemins d'échange `cycle_{CYCLE_ID}_phaseN_(input|output).json`, compare les deux côtés. Tout chemin non résolu statiquement doit porter un marqueur `# contract-dynamic` ou échoue le test. Les 13 chemins `/tmp` existants reçoivent un marqueur `# nosec B108` temporaire, en attente de la migration issue #392. 170 tests unitaires.

#### #424 — docs(memory): mémoire thématique du projet (.claude/memory/)

- **Branche** : `docs/memoire-thematique-session-2026-08-23`
- **Mergée** : 08:57 UTC
- **Résumé** : Session du 22-23/08/2026 — le bot affichait +68 USDC de gain historique alors que le PnL réel net de frais était de +11 USDC. Découverte et résolution de cet écart a produit une quinzaine de PRs et changé la compréhension du projet. Cette PR crée `.claude/memory/` avec 6 fiches thématiques : sémantique PnL net/brut, ratio gain-risque réel, frais Kraken par palier, filtre liquidité, stratégie maker, contrats prompts/scripts. Ces fiches alimentent les prochaines sessions Claude sans avoir à reconstruire les invariants depuis l'historique des PRs.

#### #425 — [M1] TP et dimensionnement net de frais

- **Branche** : `feat/issue-411-tp-sizing-net-frais`
- **Mergée** : 09:20 UTC
- **Issue liée** : #411
- **Résumé** : Feature majeure. Le bot calculait le TP et la quantité en ignorant les frais (~0,9 % aller-retour au palier Kraken actuel), et les quatre endroits de calcul du TP (`phase4_sizing.py`, `phase5_execution.py`, `maker_watcher.py`, prompt Phase 0) divergeaient. Unification avec une seule formule nette de frais : `prix_tp = entry × (1 + (stop_distance + fee_round_trip_pct) × reward_risk_ratio + fee_round_trip_pct)`. Transport de `fee_round_trip_pct` dans les ordres pending maker pour que le watcher applique la même formule au fill. Nouveau plancher de viabilité en Phase 0 : le TP recalibré ne peut jamais descendre sous `entry × (1 + 2 × fee_round_trip_pct)`. Nouveau paramètre `fee_round_trip_pct = 0,009` dans `config.json`. `reward_risk_ratio` passe de 2,0 à 1,5 (sémantique désormais nette). `min_profit_pct_take` passe de 2 % à 5 % net. 12 fichiers tests modifiés, +11 tests.

#### #426 — [M1] /perf — afficher les frais payés et rendre le gain réel plus lisible

- **Branche** : `feat/issue-396-perf-frais-rendu`
- **Mergée** : 10:42 UTC
- **Issue liée** : #396
- **Résumé** : La commande `/perf` affichait le PnL net sans ses composantes (brut, frais), avec des périodes chevauchantes (7j et 30j) et ignorait complètement la stratégie maker. Restructuration complète : 6 blocs au lieu de 4. Nouveau bloc **résultat** : décomposition brut → frais → net avec le poids des frais en %. Nouveau bloc **périodes** : fenêtres non chevauchantes 0-7j / 8-30j / au-delà. Nouveau bloc **frais** : total payé, coût moyen/trade, ventilation mesurés/estimés/non calculés. Nouveau bloc **maker** : ordres en attente, répartition maker/repli/abandonnés, économie vs tout-taker, délai médian de fill. Vocabulaire vulgarisé (TYPE_A → score faible, Quota Claude → Blocage quota). Source unique de vérité pour les ventes TP Watcher via `trade_history.json`. 28 nouveaux tests.
...
| # | Titre | Fermée à (UTC) |
|---|-------|----------------|
| #406 | [BUG] Le mode dégradé abaisse le seuil de score sans durcir aucun garde-fou | 06:57 |
| #408 | [M1] Compter les abandons du watcher maker | 06:57 |
| #416 | [REC] Remplacer Exception générique par exceptions spécifiques | 06:59 |
| #417 | [REC] Retirer l'import tg inutilisé et noqa orpheline | 06:58 |
| #418 | [REC] Remplacer chemins hardcodés /tmp/ par tempfile | 06:59 |
| #419 | [REC] Refactorer la fonction main() de fake_kraken.py | 06:59 |
| #420 | [REC] Trier les imports et retirer noqa orpheline dans test_phase1_scan.py | 06:58 |
| #409 | [BUG] Frais d'entrée jamais renseignés pendant le déploiement de #382 | 06:59 |
| #407 | [M1] Filtrer la liquidité sur le spread et sa persistance, pas un instantané | 07:05 |
| #421 | [BUG] phase6_next_cycle.py duplique fmt_local et next_4h_slot — heure décalée de 2h | 07:34 |
| #403 | [TECH] Test de cohérence des contrats prompt ↔ script | 08:46 |
| #411 | [M1] Le TP se déclenche sur du brut alors que le PnL est comptabilisé en net | 09:20 |
| #396 | [M1] /perf — afficher les frais payés et rendre le gain réel plus lisible | 10:42 |

---

### Nouveaux tickets créés (1 open)

#### #410 — [M1] Évaluer les ventes avant les achats dans un même cycle

- **Labels** : `enhancement`
- **Créé** : 06:33 UTC
- **Statut** : open
- **Contexte** : Cycle `20260822_180939` — 4 candidats valides refusés pour `max_open_positions` (4/4) alors que LINK et XBT ont été vendus dans le même cycle, terminant à 2/4 positions. Le compte de positions utilisé par la Phase 3 doit refléter l'état après les ventes du cycle courant, et non l'état initial du snapshot.

---

### Matériel Medium

**Session du 22-23/08** — La journée du 23 est la fin d'une session de crise déclenchée par l'incident TRUMP. En moins de 12 heures entre 06:57 et 10:42 UTC, 9 PRs ont été mergées couvrant 6 dimensions distinctes d'un même problème systémique : le bot prenait ses décisions sur des grandeurs brutes (TP, dimensionnement, seuils de profit) alors que sa comptabilité était en net de frais.

**Angle narratif possible** : « Le bot qui mentait à lui-même » — comment un bot de trading peut afficher +68 USDC de gains tout en ayant perdu de l'argent (+11 USDC net de frais, puis -1,91 après TRUMP). L'écart entre +68 et -1,91 est entièrement explicable par une série de décisions techniques cohérentes mais mal alignées : PnL brut présenté comme net, TP calculé sans frais, filtre liquidité sur un instantané, mode dégradé sans garde-fou RSI. La session du 23 a fermé ces 4 boucles en une journée.

**Données clés** :
- 9 PRs mergées, 13 issues fermées, 1 ticket ouvert
- Frais absorbaient 47 % des gains du TP watcher sur la période mesurée
- Incident TRUMP : risque prévu 9,22 USDC → perte réelle 13,20 USDC (+43 %), dont 2,83 USDC de glissement stop + 1,12 USDC de frais
- Spread TRUMP : 0,126 % contre 0,010-0,011 % pour XBT/SOL (12x plus large)
- PnL net cumulé : +11,11 USDC (avant TRUMP) → -1,91 USDC (après TRUMP)
...
#### #386 — [BUG] Inclure la valeur des positions ouvertes dans portfolio_total

- **Branche** : `feat/issue-385-portfolio-total`
- **Mergée** : 07:30:49 UTC
- **Issue liée** : #385
- **Résumé** : `phase0_snapshot.py` sommait déjà le prix courant de chaque position pour le P&L, mais n'exposait pas cette valeur à `portfolio_total`. Le budget disponible et le coupe-circuit `daily_loss_limit_pct` étaient donc calculés sur le seul solde USDC libre — sans prendre en compte les fonds déjà engagés. La boucle existante est réutilisée pour remonter `positions_value` dans le JSON de sortie et dans le prompt Phase 0.

#### #387 — [BUG] Corriger l'enforcement de max_open_positions en Phase 3

- **Branche** : `feat/issue-384-max-open-positions-enforcement`
- **Mergée** : 07:31:06 UTC
- **Issue liée** : #384
- **Résumé** : Bug critique découvert lors du cycle `20260821_040505` : `open_positions` était une valeur statique du snapshot Phase 0, jamais incrémentée au fil de la boucle de scoring. Résultat : avec `open_positions=1` et `max=4`, quatre coins avaient été acceptés dans le même cycle → 5 positions réelles. Le check compare désormais `open_positions + len(buy_candidates)`, sur le même modèle que la vérification de corrélation. Le `skip_detail` reflète le compte réel.

#### #391 — [BUG] Traçabilité des frais Kraken et PnL net

- **Branche** : `feat/issue-382-tracking-frais`
- **Mergée** : 11:02:35 UTC
- **Issue liée** : #382
- **Résumé** : `pnl_usdc` devient le PnL net (frais d'entrée + sortie déduits) sur les 5 points de clôture de position. L'ancien calcul brut est conservé sous `pnl_gross_usdc`. Chaque trade reçoit `entry_fee_usdc`, `exit_fee_usdc`, `fees_usdc` et `maker_or_taker` pour la traçabilité. `maker_or_taker` est désormais dérivé de `descr.ordertype` Kraken (via `maker_or_taker_from_ordertype()`) plutôt que figé à `"taker"`, ce qui prépare l'arrivée de #388. Ajoute aussi `scripts/backfill_fees.py` pour rapprocher l'historique des fills réels.

#### #394 — fix(backfill): pnl_gross_usdc reprend la valeur stockée + garde-fou de cohérence

- **Branche** : `fix/backfill-pnl-gross-coherence`
- **Mergée** : 11:17:03 UTC
- **Issue liée** : #382 (suite)
- **Résumé** : Incident de production post-#391 : `scripts/backfill_fees.py` recalculait `pnl_gross_usdc` via `(exit_price - entry_price) × quantity` au lieu de reprendre le `pnl_usdc` déjà stocké. Invisible sur 76 trades sur 77, mais le trade legacy Binance SYN (données de prix internes incohérentes depuis l'origine) avait injecté +44 USDC fictifs dans le total (+68 → +112 USDC constaté en prod). Fix : reprendre la valeur stockée telle quelle. Ajout d'un garde-fou de cohérence : si l'écart relatif dépasse 1%, le trade n'est pas modifié mais seulement signalé.

#### #395 — [BUG] Fuseau d'affichage explicite (heure locale réellement Paris)

- **Branche** : `feat/issue-393-display-timezone`
- **Mergée** : 11:21:45 UTC
- **Issue liée** : #393
- **Résumé** : `fmt_local()` utilisait `astimezone()` sans argument, donc héritait du fuseau de l'hôte. Correct sur le Mac (CEST), faux depuis la migration VPS (Etc/UTC) : les notifications Telegram affichaient l'heure UTC sous le libellé « (heure locale) ». Ajout de `display_timezone` dans `config.json`, lu via `zoneinfo` avec repli sur le comportement actuel si absent.

#### #397 — [M1] Watcher d'ordres limite maker sur les entrées (stratégie B)

- **Branche** : `feat/issue-388-maker-entry-watcher`
- **Mergée** : 16:41:42 UTC
- **Issue liée** : #388
- **Résumé** : Feature majeure. Remplace le BUY MARKET de `phase5_execution.py` par un ordre LIMIT post-only posé au bid quand `maker_entry_enabled=true` (défaut). Le nouvel ordre est enregistré dans `state/maker_pending_orders.json` ; `core/maker_watcher.py` (thread daemon, patron `tp_watcher.py`, tick 20s) l'ajuste via `kraken order amend` jusqu'à exécution ou repli selon 3 bornes : budget de concession 0,30 % (repli marché), invalidation via `price_deviation_max_pct` (abandon), timeout 60 min (repli marché). Objectif : économiser 0,30 % de frais par trade (maker 0,30 % vs taker 0,60 %), estimé à **~27,56 USDC** sur la période mesurée contre un PnL net total de +11,11 USDC. `maker_entry_enabled=false` replie intégralement sur le BUY MARKET historique. La PR intègre également 5 recommandations REC-AUTO du tech-lead (#398–#402) : extraction de handlers, types d'exceptions spécifiques, `check=False` explicite, `tempfile.mkstemp()`, annotation mypy.

#### #404 — [M1] Commande Telegram /maker — suivi du watcher d'ordres maker

- **Branche** : `feat/issue-389-commande-maker`
- **Mergée** : 17:02:21 UTC
- **Issue liée** : #389
- **Résumé** : Ajoute la commande `/maker` dans `webhook_server.py`. Retourne en moins de 5 secondes trois blocs d'information : santé du watcher (actif/inactif), liste des ordres maker en attente avec leur état courant (coin, quantité, prix bid, délai écoulé), et compteurs cumulés d'efficacité (fills réels, abandons, timeouts depuis le démarrage).
...
| # | Titre | Fermée à (UTC) |
|---|-------|----------------|
| #402 | [REC] Corriger erreur mypy : str \| None incompatible | 12:08 |
| #401 | [REC] Ajouter paramètre check= explicite aux subprocess.run() | 12:08 |
| #400 | [REC] Remplacer except Exception par types spécifiques | 12:11 |
| #399 | [REC] Remplacer chemins /tmp hardcodés par tempfile.mkstemp() | 12:09 |
| #398 | [REC] Réduire complexité de _maker_watcher_tick() : extraire handlers | 12:08 |
| #393 | [BUG] Les heures affichées "heure locale" sont en UTC depuis la migration VPS | 11:21 |
| #388 | [M1] Watcher d'ordres limite maker sur les entrées (stratégie B) | 16:41 |
| #385 | [BUG] Inclure la valeur des positions dans portfolio_total | 07:30 |
| #384 | [CONFIG] Drawdown 36% + violation max_open_positions (5>4) — ajustement requis | 07:31 |
| #382 | [BUG] PnL calculé sans soustraire les frais Kraken (fee) | 11:02 |

---

### Nouveaux tickets créés (6 open)

#### #405 — [CONFIG] max_open_positions trop bas — 7 cycles bloqués en 48h

- **Labels** : `enhancement`
- **Créé** : 20:09 UTC
- **Statut** : open
- **Contexte** : Alerte de l'agent d'analyse-config : depuis la correction de l'enforcement (#387), le gate fonctionne correctement — mais révèle que le seuil de 4 positions simultanées est désormais atteint fréquemment, bloquant des signaux valides. 7 cycles ont vu au moins un coin refusé à cause de ce plafond en 48h.

#### #403 — [TECH] Test de cohérence des contrats prompt ↔ script (chemins d'échange inter-phases)

- **Labels** : `enhancement`
- **Créé** : 15:58 UTC
- **Statut** : open
- **Contexte** : Généré après l'incident de régression PR #391 (revert des chemins `/tmp/` → `state/`) et la correction PR #397 (restauration du chemin de sortie fixe de phase5). Couvre la nécessité d'un test automatique qui valide que chaque script Python lit son fichier d'entrée au chemin exact que le prompt précédent écrit.

#### #396 — [M1] /perf — afficher les frais payés et rendre le gain réel plus lisible

- **Labels** : `enhancement`
- **Créé** : 11:32 UTC
- **Statut** : open
- **Contexte** : Complément naturel au tracking des frais (#382/#391) : la commande `/perf` doit exposer le total des frais payés (maker + taker cumulés) et distinguer visuellement PnL brut vs PnL net.

#### #392 — [TECH] Déplacer les fichiers d'échange inter-phases de /tmp/ vers state/

- **Labels** : `enhancement`
- **Créé** : 11:04 UTC
- **Statut** : open
- **Contexte** : Ticket structurel séparé du revert PR #391 : formaliser la migration une fois que le prompt (côté Claude) sera synchronisé avec le script.

#### #390 — [M1] Étendre la stratégie maker aux sorties de position

- **Labels** : `enhancement`
- **Créé** : 10:08 UTC
- **Statut** : open
- **Contexte** : Suite logique de #388/#397 : appliquer le même principe d'ordre LIMIT maker aux sorties (TP), pas seulement aux entrées.

#### #389 — [M1] Commande Telegram /maker — suivi du watcher d'ordres maker

- **Labels** : `enhancement`
- **Créé** : 10:08 UTC
- **Statut** : closed (implémenté dans PR #404 le même jour)

---

### Commits directs notables sur `main`

Commits automatiques de cycle log — 6 cycles réguliers (slots 4h) + 1 hors-slot :

| Timestamp UTC | Hash | Type |
|---|---|---|
| 07:58 | `b47d4f6` | chore: cycle log 20260822_075857 |
| 12:17 | `9dd6ac7` | chore: cycle log 20260822_120504 |
| 15:58 | `42bb31b` | chore: cycle log 20260822_155050 |
| 16:12 | `d0a15ad` | chore: cycle log 20260822_160500 |
| 18:18 | `698e4bc` | chore: cycle log 20260822_180939 |
| 20:14 | `654ef3b` | chore: cycle log 20260822_200505 |

---

### Matériau Medium

**Axe narratif principal** : Une journée en deux temps. Le matin, fiabilisation du socle : corriger deux bugs qui faussaient silencieusement la vision du portefeuille depuis des semaines (positions non comptées dans le budget, gate de concentration jamais déclenché). L'après-midi, déploiement de la première vraie stratégie d'optimisation des frais — le watcher maker — dont l'économie potentielle (~28 USDC sur la période de mesure) dépasse le PnL net total du bot sur la même période (+11 USDC).

**Angle secondaire** : L'incident du backfill (trade SYN, +44 USDC fictifs injectés en prod) illustre comment un bug invisible en dry-run peut apparaître sur un seul trade legacy aux données historiquement incohérentes — et l'importance du garde-fou de cohérence comme filet de sécurité sur les scripts de migration.

**Citation de commit potentielle** : *"maker_entry_enabled=false replie intégralement sur le comportement historique (BUY MARKET direct)"* — la phrase qui montre comment introduire un changement de stratégie sans régression pour l'utilisateur.
