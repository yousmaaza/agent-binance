...
> **Dernière mise à jour** : 2026-09-05 (PR #460)
> **Commit** : (HEAD)
...
| Kraken API (public) | Prix courants des cryptos depuis le dashboard (endpoint public `/0/public/Ticker`) | Appel direct via `urllib` stdlib depuis Railway (pas d'auth) |
| MongoDB Atlas | Persistance des cycles de trading (collections `cycles`, `dashboard_state` depuis PR #431) — lecture seule depuis le bot, lecture seule depuis le dashboard | `MONGODB_URI`, `MONGODB_DB` dans `.env` (bot) et dans Railroad env vars (dashboard) |
...
| Flask (Railway) | Dashboard web indépendant (#432) : affichage 3 onglets (Résultats, Cycles, Réglages), lecture seule MongoDB + Kraken API, auth password simple, cache TTL process-local | Hébergement sur Railway via Procfile + Nixpacks (Python 3.11, gunicorn) ; variables d'env : `MONGODB_URI`, `DASHBOARD_PASSWORD`, `DASHBOARD_SECRET_KEY` |
...
| Fonction | Ligne | Rôle |
|---|---|---|
| `safe_next_path(value)` | dashboard/auth.py:20 | Valide un chemin de redirection POST-login : n'accepte que les chemins relatifs internes (commencent par `/`, pas `//` ni `/\`, sans schéma ni netloc via `urllib.parse.urlsplit()`) — prévention faille redirection ouverte #435 ; retourne chemin validé ou `None` si rejeté |
...
| **[Dashboard — PR #432]** |  |  |
| `get_dashboard_state()` | dashboard/mongo_client.py:55 | Charge `dashboard_state` (doc MongoDB avec financials, positions, watchers, config) avec cache TTL 60s ; exceptions distinctes `MongoUnavailable` / `DashboardStateMissing` |
| `get_recent_cycles()` | dashboard/mongo_client.py:70 | Charge les N derniers cycles depuis MongoDB (limit configurable, défaut 60) avec cache TTL 60s |
| `get_prices()` | dashboard/kraken_client.py:48 | Récupère les prix courants via Kraken API publique `/Ticker` (groupé en un seul appel), cache TTL 30s, gère paires legacy (X/Z prefix) |
| `blocking_reasons()` | dashboard/analysis.py:19 | Recalcule la répartition des skip_type (TYPE_A/B/C/D) sur un ensemble de cycles, retourne breakdown + phrase motif dominant |
| `reliability_by_period()` | dashboard/analysis.py:82 | Recalcule le taux de succès (cycles sans erreur) sur 7j / 30j, génère phrase de tendance dynamique (progresse / recule / stable) |
| `build_positions()` | dashboard/viewdata.py:135 | Enrichit les positions ouvertes avec prix courants Kraken : calcule distances stop/tp et PnL % non réalisé |
| `equity_curve_points()` | dashboard/viewdata.py:44 | Génère les points SVG polyline pour la courbe d'équité cumulée |
| `build_cycle_row()` | dashboard/viewdata.py:168 | Formate un cycle Mongo brut en row de tableau : conversions timezone, extraction execution, skip_type, explanation_fr |
| `check_password()` | dashboard/auth.py:16 | Vérifie le mot de passe via `hmac.compare_digest()` (timing-safe) |
| `login_required` | dashboard/auth.py:19 | Décorateur Flask pour protéger les routes : redirige vers `/login` si session non authentifiée |
| **[Dashboard — PR #443]** |  |  |
| `weekly_note()` | dashboard/analysis.py:47 | Synthèse des 7 derniers jours entièrement recalculée (#442) : count trades clôturés, montants net/brut/frais, cycles avec action, stratégie maker ; retourne texte français à afficher, jamais figé |
| `format_price()` | dashboard/viewdata.py:14 | Arrondi lisible des prix Kraken bruts : 2 décimales au-dessus de 1, 6 en dessous, zéros de queue retirés, séparateur milliers espace fine insécable (#442) |
| `equity_curve_geometry()` | dashboard/viewdata.py:60 | Courbe d'équité + aire remplie + ligne du zéro pour SVG (#442) : retourne dict {polyline, polygon, zero_y, final_value, width, height, pad} — calculs trigonométriques non faisables en template |
| `cadence_summary()` | dashboard/viewdata.py:219 | Décompte pour légende bande de cadence : total cycles, acted, failed, idle, threshold, positions SVG — utilité a11y et étiquetage (#442) |
| `_load_state()` | dashboard/app.py:53 | Helper : chargement état dashboard + gestion erreurs Mongo, retourne (state, error_tuple) pour flux lisible |
| `_load_prices()` | dashboard/app.py:62 | Helper : chargement prix Kraken + gestion erreurs, retourne (prices_dict, kraken_error_str) |
| `_load_cycles()` | dashboard/app.py:73 | Helper : chargement cycles MongoDB récents + gestion erreurs, retourne (cycles_list, error_str) |
| `_build_results_view()` | dashboard/app.py:81 | Helper : assembly view onglet Résultats (financials, periods, equity, positions, maker, weekly_note) |
| `_build_cycles_view()` | dashboard/app.py:100 | Helper : assembly view onglet Cycles (journal, cadence, cadence_summary, blocking, reliability) |
| `_load_grid_cycles()` | dashboard/app.py:82 | Helper : charge les N cycles pour la grille avec projection Mongo optimisée (#450) — limite = 6 slots/jour × N jours + 40 cycles manuels |
| `pnl_by_period()` | dashboard/viewdata.py:93 | Dérive le PnL net de chaque jour/mois depuis la courbe cumulée `equity_curve` Mongo : la somme des périodes reconstitue exactement `net_usdc` (#450). Accepte granularité "day" ou "month" |
| `pnl_bars()` | dashboard/viewdata.py:133 | Géométrie SVG des barres divergentes autour de zéro : position, hauteur, largeur pour chaque période. Échelle symétrique autour de zéro (pertes et gains équilibrés visuellement) (#450). Inclut stats meilleur/pire/total |
| `build_cycle_grid()` | dashboard/viewdata.py:282 | Grille théorique des créneaux auto (6 par jour × N jours), chacun rattaché au cycle correspondant en base ou marqué "missing" (jamais démarré) (#450). Retourne colonnes (jours), comptes par état, stats taux réussite/erreur. Seule façon de voir les cycles qui n'ont jamais démarré |
| `_match_cycles_to_slots()` | dashboard/viewdata.py:348 | Associe chaque cycle au créneau théorique le plus proche (tolérance 1 heure). Cycles manuels près d'un créneau sont rattachés au créneau (travail accompli, peu importe le déclencheur) (#450) |
| `_grid_cell()` | dashboard/viewdata.py:368 | Calcule l'état d'une cellule grille (action/idle/error/missing) + infobulle (score, durée, coût API, ordres) (#450) |
| `_mark_date_labels()` | dashboard/viewdata.py:337 | Marque les dates à afficher (tous les 7 jours + premiers de mois) en évitant les collisions : 31/07 et 01/08 trop proches (#450) |
| `_slot_label()` | dashboard/viewdata.py:391 | Formate une heure de créneau en heure locale affichable (#450) |
| `get_cycles_for_grid()` | dashboard/mongo_client.py:91 | Charge les cycles pour la grille avec projection légère (10 champs, sans décisions/explication) pour limiter la charge Mongo (#450). Cache TTL 60s |
...
| `maker_watcher_loop()` | core/maker_watcher.py:82 | Boucle daemon du watcher d'ordres LIMIT maker : tick toutes les 20s (configurable `maker_tick_seconds`), appelle `_maker_watcher_tick()`, gère exceptions gracieusement, sleep continu |
| `_maker_watcher_tick()` | core/maker_watcher.py:88 | Logique par tick : charge `maker_pending_orders.json`, query chaque ordre Kraken, décide action (rempli → register + SL, prix invalidé → abandon, timeout/budget → repli BUY MARKET, sinon amend au bid courant) ; écrit atomiquement `maker_watcher_state.json` avec télémétrie |
| `_register_open_position()` | core/maker_watcher.py:139 | Enregistre une position remplie (maker ou taker) dans l'historique `trade_history.json` avec UUID trade_id, date ISO, prix d'entrée, stop-loss, TP cible, quantité, frais, statut `open` ; envoie notification Telegram avec label `maker`/`repli marché` |
| `_fallback_market_buy()` | core/maker_watcher.py:197 | Repli BUY MARKET : pose ordre au marché, query jusqu'à closed (3 tentatives, 1s), enregistre position via `_register_open_position()` avec `maker_or_taker="taker"` |
| `_cancel_and_resolve()` | core/maker_watcher.py:221 | Annule un ordre LIMIT et résout le remplissage partiel : query post-annule, enregistre remplissage partiel si vol_exec > 0, sinon appelle repli ou abandon selon `allow_market_fallback` |
| `_resolve_after_cancel()` | core/maker_watcher.py:237 | Résout l'état après annule : retourne `filled_partial` si vol_exec > 0, sinon `market_fallback` (si autorisé) ou `abandoned` (notification Telegram) |
| `_handle_closed_order()` | core/maker_watcher.py:261 | Traite un ordre rempli (status "closed") : acquiert le lock, enregistre la position, retourne (True, 1 fill, 0 fallback) ou (False, 0, 0) sur erreur |
| `_handle_externally_canceled()` | core/maker_watcher.py:269 | Traite un ordre annulé de façon inattendue (status "canceled" hors de notre fait) : résout sans repli marché (cas ambigu), retourne (True, 1 fill, 0 fallback, 0 abandoned) si remplissage partiel, sinon (False, 0, 0, 1 si abandoned) |
| `_handle_invalidated_price()` | core/maker_watcher.py:286 | Traite un ordre invalidé par le drift prix (distance au `scan_price` > `price_deviation_max_pct`) : annule, résout sans repli marché (signal mort), retourne (True, 1 fill, 0 fallback, 0) si remplissage partiel, sinon (False, 0, 0, 1 si abandoned) ; notification Telegram d'abandon |
| `_handle_timeout_or_concession()` | core/maker_watcher.py:303 | Traite dépassement de budget de concession (`maker_max_concession_pct`) ou timeout (`maker_timeout_seconds`) : annule et appelle repli BUY MARKET, retourne (True, 1 fill, 1 fallback, 0) si repli, sinon (False, 0, 0, 0) ; quatrième paramètre toujours 0 (no abandon car `allow_market_fallback=True`) |
| `_write_watcher_state()` | core/maker_watcher.py:47 | Écrit atomiquement `state/maker_watcher_state.json` : timestamp UTC ISO+Z du dernier tick, status, erreur optionnelle, ordres vérifiés ce tick, compteurs cumulatifs (total_ticks, total_fills, total_fallbacks, total_abandoned) ; persistence via tempfile + os.replace() ; rétrocompatibilité : lecture anciens états sans `total_abandoned` via `.get(..., 0)` |
| `load_maker_pending_orders()` | core/trade_helpers.py:107 | Charge `state/maker_pending_orders.json` (liste ordres LIMIT en attente) — retourne `[]` si absent/invalide |
...
| `attempt_maker_exit()` | core/maker_exit_watcher.py:131 | **Orchestration sortie** : annule le stop-loss de la position, pose une vente LIMIT post-only au prix ask courant. Retourne un enregistrement pour `maker_exit_pending_orders.json` ou `None` si échec (repose immédiatement le stop pour garantir protection) |
...
| `_handle_filled_order()` | core/maker_exit_watcher.py:274 | Cas 1 : ordre fermé (status "closed") → clôture position avec `exit_maker_or_taker="maker"`, retrait de liste, retourne (True, 1 fill, 0 fallback) |
| `_handle_externally_resolved()` | core/maker_exit_watcher.py:290 | Cas 2 : ordre annulé/expiré hors watcher → compte fill partiel si présent, repose stop sur reliquat (jamais d'abandon côté sortie), retourne (True, 1 fill, 0) ou (True, 0, 0) selon présence remplissage |
| `_handle_chase_end()` | core/maker_exit_watcher.py:324 | Cas 3 : délai/concession/prix redescendu dépassé → annule limite, compte fill partiel, reliquat vendu au marché avec fallback (`exit_maker_or_taker="taker"`), échecks → repose stop, retourne (True, 1, 1) si succès ou (True, 0, 0) si repose |
| `_handle_amend_order()` | core/maker_exit_watcher.py:398 | Cas 4 : ask courant ≠ prix limite → amend limite au nouvel ask (post-only), silencieux sur erreur, incrémente compteur adjustments |
| `_place_stop_loss()` | core/maker_exit_watcher.py:108 | Arrondit qty/prix au pas Kraken et pose SL. Retourne (sl_txid, protection_failed, err_msg, stop_price_rounded) — réutilisé pour pose initiale et repose d'urgence |
| `_repose_stop_and_alert()` | core/maker_exit_watcher.py:245 | Repose SL sur position existante + notification Telegram ; si repose échoue → `protection_failed=True` (position nue flaggée) |
| `load_maker_exit_pending_orders()` | core/maker_exit_watcher.py:58 | Charge `state/maker_exit_pending_orders.json` (liste ordres LIMIT en chasse) — retourne `[]` si absent/invalide |
| `save_maker_exit_pending_orders()` | core/maker_exit_watcher.py:68 | Écriture atomique `state/maker_exit_pending_orders.json` via tempfile + os.replace() |
| `_write_watcher_state()` | core/maker_exit_watcher.py:73 | Maj `state/maker_exit_watcher_state.json` : timestamp UTC ISO+Z, status, erreur, ordres checked, compteurs total_ticks/total_fills/total_fallbacks |
...
| **[Dashboard — PR #455]** |  |  |
| `_closed_trades()` | phase7_mongo.py:158 | Phase 7 — Projection étroite des ventes récentes pour l'onglet Ventes : filtres fermées via `status == "closed"`, limite 200 dernières (borne large pour 87 ventes/4 mois), tri DESC `exit_date`, transmet frais estimés et défauts de données tels quels pour transparence dashboard (#455) |
| `build_sales_view()` | dashboard/viewdata.py:586 | Orchestration complète onglet Ventes : agrégats montants/médians/effectifs, détection anomalies, déclencheurs, qualité frais — chaque métrique accompagnée d'effectif pour éviter moyennes trompeuses (#455) |
| `build_sales_rows()` | dashboard/viewdata.py:476 | Transformation chaque vente fermée en ligne affichage : conversions heures locales (`entry_local`, `exit_local`), labels motif clôture, calculs `invested`/`proceeds` en USDC (plus parlant qu'un prix unitaire coin), détestation anomalies, flag gagnante |
| `filter_sales_window()` | dashboard/viewdata.py:511 | Restreint ventes à fenêtre (7j/30j/90j/180j/tout) **avant** agrégats — garantit que bandeau brut/frais/net, motifs et journal portent tous sur même période, sinon le total contredirait la liste (#455) |
| `sale_trigger()` | dashboard/viewdata.py:643 | Map motif clôture brut → déclencheur (cycle/watcher/kraken/manuel/inconnu) : **de source de vérité fiable** (le motif enregistré à l'exécution), jamais de l'horodatage — test: 6 TP watcher tombaient dans fenêtre cycle par coïncidence sur 15, 40% de faux attachements (#455) |
| `sale_anomaly()` | dashboard/viewdata.py:647 | Détecte écarts entre ce qui devrait se passer et ce qui s'est passé (**sans OHLC**) : cible franchie mais vendue par autre mécanisme (TP n'a pas déclenché), sortie TP sous sa cible (slippage), prix sortie incohérent avec résultat. Cas réel : ADA du 21/08 cible 0,2034 franchie à 0,2072 mais TP n'a pas déclenché (signal a vendu). Trade quand même gagnant +2,33, passé inaperçu (#455) |
| `close_reason_label()` | dashboard/viewdata.py:459 | Unification motifs avec plusieurs noms : `sl_hit` + `stop_hit` + `sl` → "stop touché", `tp_watcher` + `tp_watcher_cycle` → "cible atteinte", etc. — élimine fragmentation totaux (#455) |
...
| `/maker` | `run_maker()` | Affiche le suivi du watcher d'ordres LIMIT maker : santé (dernier tick, statut OK/Lent/Erreur), ordres en cours de poursuite (prix limité, concession, ajustements), efficacité cumulée (taux maker 24h/lifetime, frais évités, délai médian) — lecture seule, temps réponse < 5s |
...
| `maker_pending_orders.json` | JSON array | Ordres LIMIT maker en attente (entrée) — chaque entrée : `trade_id`, `coin`, `pair`, `txid`, `quantity`, `scan_price`, `initial_bid_price`, `current_bid_price`, `adjustments`, `placed_at` — suivi par `maker_watcher_loop()` |
| `maker_watcher_state.json` | JSON object | État du watcher d'entrée : `last_tick` (ISO+Z), `status` (ok/warning/error), `last_error`, `orders_checked`, compteurs `total_ticks`, `total_fills`, `total_fallbacks`, `total_abandoned` |
| `maker_exit_pending_orders.json` | JSON array | Ordres LIMIT maker en attente (sortie) — chaque entrée : `trade_id`, `coin`, `pair`, `txid`, `quantity`, `stop_price`, `close_reason`, `initial_limit_price`, `current_limit_price`, `adjustments`, `placed_at` (#457) — suivi par `maker_exit_watcher_loop()` |
| `maker_exit_watcher_state.json` | JSON object | État du watcher de sortie : `last_tick` (ISO+Z), `status` (ok/warning/error), `last_error`, `orders_checked`, compteurs `total_ticks`, `total_fills`, `total_fallbacks` (#457) |
| `tp_watcher_state.json` | JSON object | État du watcher TP : `last_tick` (ISO+Z), `status` (ok/warning/error), `last_error`, `positions_checked`, compteurs `total_ticks`, `total_sales` |
...
| `reward_risk_ratio` | `1.5` | Ratio reward/risk **net de frais** (#411) : le TP cible `entry × (1 + (stop_distance + fee_round_trip_pct) × reward_risk_ratio + fee_round_trip_pct)`. Valeur défaut 1.5 (précédemment 2.0, ajusté pour frais intégrés) |
| `fee_round_trip_pct` | `0.009` | Estimation du coût aller-retour (entrée + sortie) en % du montant d'ordre (#411) — utilisée pour dimensionnement net et recalibrage TP Phase 0 (plancher de viabilité) |
...
|---|---|---|
| [#460](pr-460-garde-fou-dates.md) | 2026-09-05 | [BUG] Le garde-fou numérique de l'analyse hebdomadaire rejetait les dates dans le texte généré, bloquant 100% des générations (#458) : regex `_NUMBER_RE` affinée pour vérifier uniquement les affirmations financières (décimales ou entiers suivis d'une unité %, USDC, USD, $, €, EUR). Un entier nu (date, durée, ordinal) n'est plus capturé. Limite assumée documentée : effectifs sans unité (« 27 trades clôturés ») ne sont plus vérifiés (indistinguable d'un quantième en prose sous 31, choisir entre comptes ou dates était nécessaire). Scénario du 31/08 rejoué en tests → texte avec date passe désormais. Couverture garde-fou < 35% maintenue. Suite 455 tests PASS (+10 couvrant extraction et assouplissement). |
| [#457](pr-457-extend-maker-exit-strategy.md) | 2026-09-05 | [M1] Étendre la stratégie maker aux sorties de position (#390) : nouveau module `core/maker_exit_watcher.py` (daemon thread, 512 lignes) suivant les ventes LIMIT post-only vers le haut (TP watcher, Phase 0 profit) — **contrainte Kraken** : stop immobilise le solde, d'où annulation préalable du SL, repose immédiate d'urgence en cas d'échec ultérieur (limite ou marché). Arbre de décision par tick : rempli → clôture `exit_maker_or_taker="maker"`, prix redescendu au stop/délai/concession → annule limite + vente marché + repose SL (fallback jamais d'abandon côté sortie). `tp_watcher.py` + `phase0_profit.py` : appel `attempt_maker_exit()` remplace SELL MARKET direct quand `maker_exit_enabled=true`. État persistant : `state/maker_exit_pending_orders.json` + `state/maker_exit_watcher_state.json`. Config : `maker_exit_enabled`, `maker_exit_timeout_seconds` (600s), `maker_exit_max_concession_pct` (0.3%). Tests : 400 lignes `test_maker_exit_watcher.py` couvrant 8 cas clés (fill/timeout/redescendu/échecss), 444 tests PASS au total, zéro régression suite `test_phase0_profit.py` + `test_tp_watcher.py` + `test_webhook_server_routing.py`. |
| [#456](pr-456-onglet-ventes-historique-sorties.md) | 2026-09-03 | [M1] Onglet Ventes — historique des sorties dans le dashboard (#455) : ajout module `viewdata.py` (240 lignes) orchestrant agrégats, filtrage fenêtres (7j/30j/90j/180j/tout), détection anomalies (cible franchie non-déclenchée, sortie TP sous cible), classification déclencheurs fiables (cycle/watcher/kraken/manuel — de source motif clôture jamais horodatage, car 40% TP watcher tombent coïncidence dans fenêtre cycle). Phase 7 expose `_closed_trades()` (200 dernières ventes, frais estimés préservés) dans `dashboard_state.closed_trades`. Onglet survit rechargement via query string `periode=` + `tab=ventes`. Tableau journal complet + bandeau brut/frais/net + barres divergentes motifs + durées médians + anomalies (3 sorties non-plan : ADA franchit cible mais signal vend). Données : 87 ventes historiques, 33 frais estimés, 3 anomalies détectées, 429 tests PASS (+205 viewdata/phase7 nouveaux). |
| [#454](pr-454-analyse-hebdo.md) | 2026-08-29 | [M1] Analyse hebdomadaire rédigée par Claude dans le dashboard : génération d'une analyse textuelle à partir des trades et cycles de la semaine ISO, déclenchée lundi 00:10 UTC dans `main_loop()` comme l'auto-scheduler 4h, avec double idempotence (slot horaire + key Mongo). Garde-fou numérique strict : tout nombre du texte généré doit être retrouvé dans la charge utile transmise (tel quel ou dérivé par somme/pourcentage/différence) — défaillance du contrôle ou de l'appel Claude = zéro document écrit, repli sur l'analyse déterministe `analysis.weekly_note()` clairement signalée. Fenêtre adaptative 7j/30j si < 10 trades. Novo module `core/weekly_analysis.py` (416 lignes) : 9 fonctions de sélection fenêtre, agrégats, signification statistique, appel Claude isolé, contrôle numérique. Extensions `timing.py` : `next_weekly_slot()`, `iso_week_key()`. Config `llm.py` : `WEEKLY_ANALYSIS_CLI_FLAGS` Sonnet 4.6 + timeout 120s. MongoDB : collection `weekly_analysis` avec `find_weekly_analysis()`, `save_weekly_analysis()`, `find_cycles_since()`. Dashboard : `get_latest_weekly_analysis()` + `build_weekly_analysis_view()` avec affichage fallback explicite, window_days/window_widened visibles. Template : bloc "Note de la semaine" enrichi. Tests : 351 lignes `test_weekly_analysis.py` couvrant contrôle numérique (rejet nombre inventé), idempotence (une seule génération/semaine), élargissement fenêtre, replis sur erreur. 13 tests nouveaux au total ; suite 397/397 PASS. |
| [#451](pr-451-pnl-charts.md) | 2026-08-29 | [M1] Graphe de PnL par jour/mois et grille d'état des cycles (#450) : remplacement bande de cadence SVG illisible par graphiques divergents autour du zéro (jour/mois) + grille théorique des 180 créneaux 4h/30j avec détection cycles "jamais démarrés" (invisibles à Mongo) ; 7 nouvelles fonctions dans `viewdata.py` (`pnl_by_period()`, `pnl_bars()`, `build_cycle_grid()`, `_match_cycles_to_slots()`, `_grid_cell()`, `_mark_date_labels()`, `_slot_label()`), projection Mongo optimisée `_GRID_PROJECTION` dans `mongo_client.py` (10 champs, sans décisions), 65 lignes JS pour infobulles jour/mois/grille au survol/focus clavier, 68 lignes CSS `.pnl`+`.cgrid` ; test clé `test_pnl_by_period_sums_to_net_usdc()` valide cohérence (somme PnL = net_usdc exactement) ; suppression code cadence mort ; 35 tests ajoutés (−4 obsolètes) = 31 nets ; suite 362/362 PASS |
...
| [#433](pr-433-dashboard-state-mongo.md) | 2026-08-28 | [M1] Publier l'état du bot dans MongoDB pour le dashboard : extension Phase 7 écrivant un document unique `dashboard_state` avec instantané positions ouvertes, agrégats financiers (brut/frais/net global + par période 0-7j/8-30j/30j+), PnL par coin, courbe d'équité réduite à 1 point/jour, état des surveillants (maker/TP watcher), sous-ensemble whitelisté config.json ; document dérivé jamais source de vérité (trade_history.json fait foi) ; 10 tests nouveaux couvrant structure, agrégations, edge cases → suite 236/236 PASS |
| [#429](pr-429-plafond-tp.md) | 2026-08-28 | [M1] Plafonner le TP à ce que le marché délivre réellement : ajout du paramètre `max_tp_pct` (0.06, soit 6%) plafonnant absolument la cible de prise de profit indépendamment de la résistance 4h. Historique : hausse médiane +4.9% (90e centile +9.3%), aucune cible >8% jamais atteinte (0/4), alors que les cibles mécaniques montaient jusqu'à 14%. Appliqué de façon identique aux quatre emplacements : phase4_sizing.py, phase5_execution.py, maker_watcher.py, bloc recalibrage Phase 0 du prompt. Conflit plancher/plafond (#411) traité explicitement : le plancher prime si plafond le ramène sous viabilité. Écart fin détecté entre `min_profit_pct_take` (5%) et `max_tp_pct` (6%) — à clarifier dans une PR sépaée. Suite complète : 226 tests PASS (+13 nouveaux). |
| [#426](pr-426-perf-frais-rendu.md) | 2026-08-23 | [M1] /perf — restructuration complète du rapport : décomposition brut → frais → net avec poids %, périodes non chevauchantes (0-7j / 8-30j / au-delà), nouveau bloc Frais payés (mesurés/estimés/non calculés), nouveau bloc Stratégie maker (ordres en attente, répartition maker/repli/abandonnées, économie, délai médian), réconciliation TP Watcher (source unique `trade_history.json`), ajout positions annulées, vulgarisation vocabulaire (TYPE_A→"score faible", Quota Claude→"Blocage quota"). Identité algébrique : brut = net + frais_connus. Tests : 28 nouveaux pour perf.py, suite complète 213/213 PASS (+10 vs #425). |
| [#425](pr-425-m1-tp-net-frais.md) | 2026-08-23 | [M1] TP et dimensionnement net de frais : unification des quatre calculs de TP (phase4_sizing.py, phase5_execution.py, maker_watcher.py, bloc RECALIBRAGE TP du prompt Phase 0) autour d'une seule formule nette de frais `TP = entry × (1 + (stop_distance + fee_round_trip_pct) × reward_risk_ratio + fee_round_trip_pct)`. Dimensionnement net : `quantite = risk_usdc / (entry × (stop_distance + fee_round_trip_pct))`. Changements config : `reward_risk_ratio` passe de 2.0 à 1.5 (net de frais), nouvelle clé `fee_round_trip_pct` = 0.009 (#411), `min_profit_pct_take` de 2.0 à 5.0 (évaluation nette Phase 0). Plancher de viabilité Phase 0 : recalibrage TP via résistance 4h ne descend jamais sous `entry × (1 + 2×fee_round_trip_pct)` pour prévenir les TP perdants. Transport `fee_round_trip_pct` dans `maker_pending_orders.json` pour cohérence TP au fill. Évaluation profit Phase 0 nette estimée. Nouveau test `test_phase0_recalibrate_tp_floor.py` (4 cas) reproduisant l'algorithme prompt. 185 tests, 185/185 PASS (+10). |
| [#423](pr-423-test-coherence-prompt-script.md) | 2026-08-23 | [TECH] Test de cohérence des contrats prompt / script : nouveau test `tests/test_prompt_script_contract.py` (435 lignes, 5 tests) validant statiquement que les chemins d'échange (`cycle_{CYCLE_ID}_phaseN_(input\|output).json`) restent synchronisés entre scripts Python (notation f-string `{CYCLE_ID}`) et prompts texte (notation `__CYCLE_ID__` substituée). Extraction AST plutôt que regex brute pour résoudre expressions composées (`PROJECT_DIR`, `os.path.join`, f-strings). Inversement du fardeau de la preuve : tout chemin non résoluble statiquement déclenche erreur sauf s'il porte le marqueur `# contract-dynamic`. Ajout du marqueur `# nosec B108` sur 13 chemins `/tmp` existants pour documenter explicitement l'exemption de bandit (migration `/tmp` → `state/` reportée à PR #392). MEMORY.md documenté avec contexte des 3 incidents du 2026-08-22 (PR #391, PR #397, bug #385) — seule cette PR ajoute un garde-fou régressif. 170 tests, 170/170 PASS. |
| [#422](pr-422-phase6-fuseau.md) | 2026-08-23 | [BUG] Élimination de la duplication dans `phase6_next_cycle.py` (#421) : le script réimplémentait son propre calcul du slot 4h UTC (`next_4h_slot()`) et son propre formatage d'heure locale (`.astimezone()` sans argument), le maintenant en dehors du correctif #395 qui a centralisé ces deux fonctionnalités dans `core/timing.py` avec support explicite du fuseau configuré `display_timezone`. Manifestation du bug : sur VPS Hostinger en UTC, `.astimezone()` retourne l'heure UTC inchangée, affichant `08:05 (heure locale)` au lieu de `10:05` en été Paris (+02:00). Refactoring minimaliste : `phase6_next_cycle.py` réduit de 26 à 15 lignes, appels `next_4h_slot()` et `fmt_local()` centralisés. Tests adaptés pour patcher `core.timing.datetime` ; classe de base `_DisplayTimezoneTestCase` fixe fuseau à Europe/Paris indépendamment de la machine ; test de régression #421 couvre cas UTC/Paris. 5 tests, 166/166 PASS. |
| [#414](pr-414-filtre-liquidite.md) | 2026-08-23 | [M1] Filtrer la liquidité sur le spread et sa persistance, pas un instantané (#407) : ajout de deux filtres Phase 1 — spread ask/bid (extraction des champs `a`/`b` de `kraken ticker`, calcul en %) exclut paires avec spread > `max_spread_pct` (0.0008 = 0,08%, configurable) ; persistance du volume via appels `kraken ohlc` sur candidats volume+spread filtrés (appels réseau limités à ~6 paires au lieu de 46) vérifie majorité des 6 périodes 4h récentes (≥4/6) tiennent `min_volume_usdc / 6` → écarte pics isolés type TRUMP (x27 volume une nuit, spread 12x plus large que XBT/SOL, stop-loss glissé 2,3%) ; seuil 0,08% calibré pour retenir ADA (+10,57 USDC net, pièce la plus profitable historique) tout en excluant BNB/LINK/TRUMP (spread 2-3x supérieures aux majors) ; `portfolio_coins` (XBT/XRP/SOL) inclus systématiquement indépendamment des seuils ; deux nouveaux paramètres `config.json` : `max_spread_pct: 0.0008`, `volume_persistence_periods: 6` ; refactoring `fake_kraken.py` dispatch dict + support `ohlc` ; 5 tests totaux (+2 `TestSpreadFilter`/`TestVolumePersistenceFilter`), 154/154 OK. |
| [#415](pr-415-frais-entree-manquants.md) | 2026-08-23 | [BUG] Frais d'entrée manquants sur positions ouvertes et trades clôturés (#409) : trois flux disjoints dans `backfill_fees.py` — (1) positions ouvertes ne reçoivent que `entry_fee_usdc` mesuré via `entry_order_id`, jamais champ de clôture touché, (2) trades clôturés avec `exit_fee_usdc` présent mais entrée manquante → appel `compute_net_pnl()` pour recalculer frais/PnL nets sans re-matcher l'exit, (3) flux nominal inchangé. Nouvelle fonction `_resolve_entry_fee()` centralise logique mesure/estimation, compteur `open_entries_backfilled` dédié. Garde-fou `_coherence_diagnostic()` amélioré : compare `pnl_gross_usdc` en priorité (si présent, trade dont exit captée en direct) sinon fallback `pnl_usdc` (comportement legacy #382). Tests : 4 cas nouveaux + 1 fixture ajustée, suite 155/155 PASS. |
...
| [#412](pr-412-compter-abandons-watcher.md) | 2026-08-23 | [M1] Compteur d'ordres abandonnés au watcher maker : ajout `total_abandoned` dans `state/maker_watcher_state.json` à côté de `total_fills` et `total_fallbacks`, incrémenté quand un ordre est invalidé (drift prix) ou annulé externement sans remplissage. Propagation `abandoned_delta` à travers les 3 handlers `_handle_externally_canceled()`, `_handle_invalidated_price()`, `_handle_timeout_or_concession()` — symétrie totale avec pattern `fills_delta`/`fallbacks_delta`. Rétrocompatibilité : lecture anciens états via `.get()` défaut 0. Tests : 42/42 PASS (+4 nouveaux couvrant incrément et rétrocompatibilité). Périmètre XS : compteur seul, affichage `/maker` reporté au ticket #396 (rendu des KPIs). |
