...
| `reliability_by_period()` | dashboard/analysis.py:58 | Recalcule le taux de succès (cycles sans erreur) sur 7j / 30j, génère phrase de tendance dynamique (progresse / recule / stable) |
| `build_positions()` | dashboard/viewdata.py:64 | Enrichit les positions ouvertes avec prix courants Kraken : calcule distances stop/tp et PnL % non réalisé |
| `equity_curve_points()` | dashboard/viewdata.py:30 | Génère les points SVG polyline pour la courbe d'équité cumulée |
| `build_cycle_row()` | dashboard/viewdata.py:97 | Formate un cycle Mongo brut en row de tableau : conversions timezone, extraction execution, skip_type, explanation_fr |
