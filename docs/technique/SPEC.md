...
| Binance CLI | Consultation portefeuille, passage ordres spot (BUY MARKET + OCO SELL) | profil `agent-profile` dans `~/.binance-cli/` |
...
| Fichier | Type | Rôle |
|---|---|---|
| `trade_history.json` | JSON array | Source de vérité des trades : chaque entrée contient `trade_id`, `coin`, `side`, `signal_score`, `entry_price`, `stop_price`, `tp_price`, `quantity`, `risk_usdc`, IDs des ordres (entry/stop/tp/order_list), `protection_failed` (booléen, `True` si l'OCO a échoué après le BUY MARKET), `status` (open/closed), `pnl_usdc`, `pnl_pct`, dates |
...
| [#39](pr-39-phase5-buy-market-oco-protection.md) | 2026-05-21 | Phase 5 : suppression de `order-list-otoco` → séquence `order-market` BUY + `order-list-oco` SELL standalone calculé sur `actual_entry` (prix fill réel) ; nouveau champ `protection_failed` dans `trade_history.json` ; routine idempotente de rattrapage en Phase 0 |
