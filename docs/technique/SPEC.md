...
| `_handle_externally_canceled()` | core/maker_watcher.py:277 | Traite un ordre annulé de façon inattendue (status "canceled" hors de notre fait) : résout sans repli marché (cas ambigu), retourne (True, 1 fill, 0 fallback) si remplissage partiel, sinon (False, 0, 0) |
| `_handle_invalidated_price()` | core/maker_watcher.py:293 | Traite un ordre invalidé par le drift prix (distance au `scan_price` > `price_deviation_max_pct`) : annule, résout sans repli marché (signal mort), notification Telegram d'abandon |
| `_handle_timeout_or_concession()` | core/maker_watcher.py:309 | Traite dépassement de budget de concession (`maker_max_concession_pct`) ou timeout (`maker_timeout_seconds`) : annule et appelle repli BUY MARKET (avec télémétrie `fallbacks_delta=1`) |
| `_write_watcher_state()` | core/maker_watcher.py:47 | Écrit atomiquement `state/maker_watcher_state.json` : timestamp UTC ISO+Z du dernier tick, status, erreur optionnelle, ordres vérifiés ce tick, compteurs cumulatifs (total_ticks, total_fills, total_fallbacks) ; persistence via tempfile + os.replace() |
